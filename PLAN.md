...
- Add a new `article_topics` table with `article_id`, `topic`, `relevance_score`, and `matched_at`. Stop relying on a single `articles.topic` value for topic filtering.
...
- `GET /summaries` response should include:
  - `topic`, `country`, `source`, `article_count`, `generated_at`
  - `sources_used`
  - `summary_headline`
  - `summary_text`
  - `key_points`
...
- Existing drift/brief endpoints should keep working, but topic-filtered queries should move to `article_topics` wherever correctness matters.
